...
get '/posts/:post_id/comments/hierachy' do
  ...
  root_comments = Comment.where(post_id: params[:post_id], root_comment_id: nil).includes(:subsidiary_comments)

  content_type :json
  root_comments.to_json(include:
                        {user: {only: user_attributes},
                         subsidiary_comments: {include: {user: {only: user_attributes}}}
  })
end
